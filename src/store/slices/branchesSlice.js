...
export const fetchBranch = createAsyncThunk('branches/fetchBranches', async(_, thunkApi) => {
    try{
        const response = await api.get(API_URL, {withCredentials: true});
        return response.data;
    }catch(error){
        return thunkApi.rejectWithValue(error.response?.data || 'Error fetching branch');
    }
});

export const createBranch = createAsyncThunk('branch/createBranch', async(data, thunkApi) => {
    try{
        const response = await api.post(API_URL, data, {withCredentials: true});
        return response.data;
    }catch(error){
        return thunkApi.rejectWithValue(error.response?.data || "Error creating branch")
    }
})
...
const branchSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        .addCase(updateBranch.fulfilled, (state, action) => {
            const index = state.branches.findIndex((c) => c.id === action.payload.id);
            if (index !== -1) {
